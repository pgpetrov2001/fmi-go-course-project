...
func GetPhotoMiddleware(d app.DAO, next http.Handler) http.Handler {
	return GetIdParamMiddleware("photoId", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		photoId, _ := r.Context().Value("photoId").(uint)
		photo, err := d.GetPhoto(photoId)
		if err != nil {
			http.Error(w, fmt.Sprintf("Could not get photo with id %d", photoId), http.StatusBadRequest)
			return
		}
